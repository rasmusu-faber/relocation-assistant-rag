...
- **Re-ranking:** optional cross-encoder (`ms-marco-MiniLM-L-6-v2`) re-ordering the retrieved pool
...
All three run on local embeddings only — no LLM/API key required. On the distinct
corpus (16 docs, 16 questions) the shipped config scores **hit-rate@4 100%**,
**hit-rate@1 94%**, **MRR@4 0.97**. A second, deliberately harder eval set of
confusable *sibling* documents (`eval/eval_set_hard.jsonl`) stress-tests ranking
quality — see [Re-ranking, measured](#re-ranking-measured).
...
regardless of chunk size, while the default `all-MiniLM-L6-v2` mis-ranks a question
or two at the smaller chunk sizes and only reaches hit@1 100% at chunk size 1200.
...
A bi-encoder scores the question and each passage *independently*, so it can only
rank on coarse similarity. A **cross-encoder** re-reads each `(question, passage)`
pair jointly and ranks far more accurately — but is too slow to run over a whole
corpus. The standard fix is two stages: the retriever fetches a wider candidate
pool cheaply, then the cross-encoder re-orders it down to top-k
(`app/rag/reranker.py`, enabled with `RERANK_ENABLED=true`).

Re-ranking can only re-order what retrieval already found, so it earns its keep
where several documents look alike. That is exactly what the harder eval set
provides — confusable *sibling* documents (e.g. health insurance for employees vs.
the self-employed vs. voluntary vs. family members). Measured with
`python -m eval.rerank_eval`:

<!-- generated by: python -m eval.rerank_eval  (see eval/rerank_results.md) -->
...
**Reading:** on the distinct corpus re-ranking changes nothing — there is no
ambiguity to resolve. On the confusable siblings it lifts hit@1 from **72% → 89%**
and MRR from **0.85 → 0.94**, without moving hit@4 (the right document was already
in the pool; the cross-encoder just pulls it to the top). Reporting *both* regimes
is the point: it shows where re-ranking helps and, just as honestly, where it does
not.
...
│       ├── reranker.py    # cross-encoder re-ranking of the candidate pool
│       ├── generator.py   # LLM provider abstraction (Ollama / OpenAI-compatible)
│       └── pipeline.py    # retrieve (+ optional rerank) + generate → answer
├── frontend/streamlit_app.py
├── eval/{run_eval.py, sweep.py, rerank_eval.py, groundedness.py, eval_set*.jsonl}
